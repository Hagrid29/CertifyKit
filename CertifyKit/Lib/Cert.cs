...
namespace CertifyKit
{
    ...
    class Cert
    {
        ...
        //only work on server 2016+
        public static int IssuePendingCert(string CA, int requestId)
        {
            CERTADMINLib.ICertAdmin2 certAdmin = new CERTADMINLib.CCertAdmin();
            return certAdmin.ResubmitRequest(CA, requestId);
        }
    }
}