...
namespace CertifyKit.Lib
{
    class LdapOperations
    {
        ...
        // CertifyKit request /template:xxx /alter => alter template
        public class TemplateDTO
        {
            public object mspkiEnrollmentFlag { get; set; }
            public object mspkiCertificateNameFlag { get; set; }
            public object flags { get; set; }
            public object mspkiCertificateApplicationPolicy { get; set; }
            public object pkiextendedkeyusage { get; set; }
            public object pkidefaultkeyspec { get; set; }
        ...
        public string GetDC()
        {
            ...
            return $"{rootDse.Properties["dnsHostName"][0]}";
        }
        public string GetDomain()
        {
            var rootDse = _searchOptions.Domain == null
                ? new DirectoryEntry("LDAP://RootDSE")
                : new DirectoryEntry($"LDAP://{_searchOptions.Domain}/RootDSE");
            return $"{rootDse.Properties["ldapServiceName"][0]}".Split('@')[1];
        }
        ...
        // CertifyKit request /template:xxx /alter => alter template
        public TemplateDTO AlterTemplate(string templateName, TemplateDTO target)
        {
            DirectoryEntry templateDirEnt = new DirectoryEntry($"LDAP://{LdapServer}CN={templateName}, CN=Certificate Templates,CN=Public Key Services,CN=Services,{ConfigurationPath}");

            TemplateDTO bkup = new TemplateDTO()
            {
                mspkiEnrollmentFlag = templateDirEnt.InvokeGet("mspki-enrollment-flag"),
                mspkiCertificateNameFlag = templateDirEnt.InvokeGet("mspki-certificate-name-flag"),
                flags = templateDirEnt.InvokeGet("flags"),
                mspkiCertificateApplicationPolicy = templateDirEnt.InvokeGet("mspki-certificate-application-policy"),
                pkiextendedkeyusage = templateDirEnt.InvokeGet("pkiextendedkeyusage"),
                pkidefaultkeyspec = templateDirEnt.InvokeGet("pkidefaultkeyspec")
            };

            if (target.mspkiEnrollmentFlag != null)
                templateDirEnt.InvokeSet("mspki-enrollment-flag", target.mspkiEnrollmentFlag);
            else
                templateDirEnt.Properties["mspki-enrollment-flag"].Clear();
            if (target.mspkiCertificateNameFlag != null)
                templateDirEnt.InvokeSet("mspki-certificate-name-flag", target.mspkiCertificateNameFlag);
            else
                templateDirEnt.Properties["mspki-certificate-name-flag"].Clear();
            if (target.flags != null)
                templateDirEnt.InvokeSet("flags", target.flags);
            else
                templateDirEnt.Properties["flags"].Clear();
            if (target.mspkiCertificateApplicationPolicy != null)
                templateDirEnt.InvokeSet("mspki-certificate-application-policy", new object[] { target.mspkiCertificateApplicationPolicy });
            else
                templateDirEnt.Properties["mspki-certificate-application-policy"].Clear();
            if (target.pkiextendedkeyusage != null)
                templateDirEnt.InvokeSet("pkiextendedkeyusage", new object[] { target.pkiextendedkeyusage });
            else
                templateDirEnt.Properties["pkiextendedkeyusage"].Clear();
            if (target.pkidefaultkeyspec != null)
                templateDirEnt.InvokeSet("pkidefaultkeyspec", target.pkidefaultkeyspec);
            else
                templateDirEnt.Properties["pkidefaultkeyspec"].Clear();
            templateDirEnt.CommitChanges();
